...
OTRON_HTTP_ORIGIN ?= http://localhost:15000
...
start: build-otron-image
	...
	@if [ "$(WEBUI)" = "true" ]; then \
		$(MAKE) -C ../../ start-webui ORCHESTRON_API_ORIGIN="$(OTRON_HTTP_ORIGIN)"; \
	fi
...
send-config-async:
	curl -X PUT -H "Content-Type: application/yang-data+xml" -H "Async: true" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/restconf/data

.PHONY: send-config-wait
send-config-wait:
	curl -X PUT -H "Content-Type: application/yang-data+xml" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/restconf/data

.PHONY: send-config-json-async
send-config-json-async:
	curl -X PUT -H "Content-Type: application/yang-data+json" -H "Async: true" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/restconf/data

.PHONY: send-config-json-wait
send-config-json-wait:
	curl -X PUT -H "Content-Type: application/yang-data+json" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/restconf/data
...
.PHONY: delete-config
delete-config:
	curl -X DELETE $(OTRON_HTTP_ORIGIN)/restconf/data/netinfra:netinfra/routers=STO-CORE-1
