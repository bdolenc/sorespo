...
send-config-async:
	curl -f -X PATCH -H "Content-Type: application/yang-data+xml" -H "Async: true" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/restconf/data

.PHONY: send-config-wait
send-config-wait:
	curl -f -X PATCH -H "Content-Type: application/yang-data+xml" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/restconf/data

.PHONY: send-config-json-async
send-config-json-async:
	curl -f -X PATCH -H "Content-Type: application/yang-data+json" -H "Async: true" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/restconf/data

.PHONY: send-config-json-wait
send-config-json-wait:
	curl -f -X PATCH -H "Content-Type: application/yang-data+json" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/restconf/data

.PHONY: send-config-tmf640
send-config-tmf640:
	curl  -k -sS -X POST -H "Content-Type: application/json" -H "Accept: application/json" -d @$(FILE) $(OTRON_HTTP_ORIGIN)/tmf-api/ServiceActivationAndConfiguration/v4/service | jq '.'
...
send-config-tmf640-stream:
	...
	printf '%s\n' "$$json_services" | while IFS= read -r service; do \
		response=$$(curl -f -k -sS -X POST -H "Content-Type: application/json" -H "Accept: application/json" -d "$$service" $(OTRON_HTTP_ORIGIN)/tmf-api/ServiceActivationAndConfiguration/v4/service); \
		printf '%s\n' "$$response" | jq '.'; \
	done

.PHONY: get-config-tmf640
get-config-tmf640:
	curl -k -sS -H "Accept: application/json" $(OTRON_HTTP_ORIGIN)/tmf-api/ServiceActivationAndConfiguration/v4/service$(if $(ID),/$(ID),) | jq '.'

.PHONY: get-tmf633-service-catalog
get-tmf633-service-catalog:
	curl -k -sS -H "Accept: application/json" $(OTRON_HTTP_ORIGIN)/tmf-api/serviceCatalogManagement/v4/serviceCatalog$(if $(ID),/$(ID),) | jq '.'

.PHONY: get-tmf633-service-category
get-tmf633-service-category:
	curl -k -sS -H "Accept: application/json" $(OTRON_HTTP_ORIGIN)/tmf-api/serviceCatalogManagement/v4/serviceCategory$(if $(ID),/$(ID),) | jq '.'

.PHONY: get-tmf633-service-candidate
get-tmf633-service-candidate:
	curl -k -sS -H "Accept: application/json" $(OTRON_HTTP_ORIGIN)/tmf-api/serviceCatalogManagement/v4/serviceCandidate$(if $(ID),/$(ID),) | jq '.'

.PHONY: get-tmf633-service-specification
get-tmf633-service-specification:
	curl -k -sS -H "Accept: application/json" $(OTRON_HTTP_ORIGIN)/tmf-api/serviceCatalogManagement/v4/serviceSpecification$(if $(ID),/$(ID),) | jq '.'

.PHONY: get-config-restconf
get-config-restconf:
	curl -f -sS -H "Accept: application/yang-data+xml" $(OTRON_HTTP_ORIGIN)/restconf/data

.PHONY: get-config-restconf-json
get-config-restconf-json:
	curl -f -sS -H "Accept: application/yang-data+json" $(OTRON_HTTP_ORIGIN)/restconf/data | jq '.'

.PHONY: get-config0 get-config1 get-config2 get-config3
get-config0 get-config1 get-config2 get-config3:
	curl -H "Accept: application/yang-data+xml" $(OTRON_HTTP_ORIGIN)/layer/$(subst get-config,,$@)
...
get-config-json0 get-config-json1 get-config-json2 get-config-json3:
	@curl -H "Accept: application/yang-data+json" $(OTRON_HTTP_ORIGIN)/layer/$(subst get-config-json,,$@)
...
get-config-adata0 get-config-adata1 get-config-adata2 get-config-adata3:
	@curl -H "Accept: application/yang-data+acton-adata" $(OTRON_HTTP_ORIGIN)/layer/$(subst get-config-adata,,$@)?loose=$(LAYER_CONFIG_LOOSE)
...
$(addprefix get-target-,$(ROUTERS_XR) $(ROUTERS_CRPD) $(ROUTERS_SRL)):
	@curl $(OTRON_HTTP_ORIGIN)/device/$(call upper,$(subst get-target-,,$@))/target?format=$(DEVICE_CONFIG_FORMAT)
...
$(addprefix get-running-,$(ROUTERS_XR) $(ROUTERS_CRPD) $(ROUTERS_SRL)):
	@curl $(OTRON_HTTP_ORIGIN)/device/$(call upper,$(subst get-running-,,$@))/running?format=$(DEVICE_CONFIG_FORMAT)
...
$(addprefix get-diff-,$(ROUTERS_XR) $(ROUTERS_CRPD) $(ROUTERS_SRL)):
	@curl $(OTRON_HTTP_ORIGIN)/device/$(call upper,$(subst get-diff-,,$@))/diff?format=$(DEVICE_CONFIG_FORMAT)
...
$(addprefix resync-,$(ROUTERS_XR) $(ROUTERS_CRPD) $(ROUTERS_SRL)):
	@curl $(OTRON_HTTP_ORIGIN)/device/$(call upper,$(subst resync-,,$@))/resync

.PHONY: delete-config
delete-config:
	curl -f -X DELETE $(OTRON_HTTP_ORIGIN)/restconf/data/netinfra:netinfra/router=STO-CORE-1
...
test-restconf-get:
	curl -sS -f -H "Accept: application/yang-data+json" $(OTRON_HTTP_ORIGIN)/restconf/data/netinfra:netinfra/router=AMS-CORE-1 | jq '.["netinfra:router"][0].name' | grep -q "AMS-CORE-1"
